...
const Menu = () => {
  ...
  const [isLoadingPrevBtn, setIsLoadingPrevBtn] = useState(false);
      ...
      <DashboardLayout>
        <div className="p-6 space-y-6 bg-gray-50/50 min-h-full">
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <span
                    className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-medium text-white shadow-sm ${item.is_available ? "bg-green-500" : "bg-gray-500"}`}
                  >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-sm font-medium text-gray-700 transition-colors group/btn"
                    >
                      {item.is_available ? (
                        <>
          ...
          <div className="flex justify-center items-center">
            <button
              className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600 transition-colors shadow-sm shadow-orange-200 disabled:cursor-not-allowed disabled:bg-orange-500/50"
              disabled={prevPageURL === null || isLoadingPrevBtn}
              onClick={async () => {
                setIsLoadingPrevBtn(true);
                const parsedURL = new URL(prevPageURL);
                const page = parsedURL.searchParams.get("page");

                try {
                  const { data } = await api.get(`/admin/menus?page=${page}`);
            ...
            <button
              className="ml-5 flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600 transition-colors shadow-sm shadow-orange-200 disabled:cursor-not-allowed disabled:bg-orange-500/50"
              disabled={nextPageURL === null || isLoadingNextBtn}
              onClick={async () => {
