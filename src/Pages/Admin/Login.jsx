...
const Login = () => {
  ...
  const [isDeliveryAgent, setIsDeliveryAgent] = useState(null);
  ...
  useEffect(() => {
    if (staffType !== "staff" || isDeliveryAgent !== false) return;
    let cancelled = false;
  ...
  }, [staffType, isDeliveryAgent]);

  const handleStaffTypeChange = (type) => {
    setStaffType(type);
    setIsDeliveryAgent(null);
    setOutletId("");
    setOutlets([]);
    setError("");
  };

  const handleDeliveryAgentChange = (value) => {
    setIsDeliveryAgent(value);
    setOutletId("");
    setOutlets([]);
    setError("");
  };
  ...
  const handleLogin = async (e) => {
    ...
    if (staffType === "staff" && isDeliveryAgent === false && !outletId) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {staffType === "staff" && (
              <div>
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => handleDeliveryAgentChange(true)}
                  ...
                  <button
                    type="button"
                    onClick={() => handleDeliveryAgentChange(false)}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${
            ...
            {staffType === "staff" && isDeliveryAgent === false && (
              <div>
