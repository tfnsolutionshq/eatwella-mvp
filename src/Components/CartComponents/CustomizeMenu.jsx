...
function CustomizeMenu({
  ...
  const { drinkItems, sideItems } = useMemo(() => {
    ...
    const withCategory = allMenus.filter(
      (item) => item && (item.category || item.category_name),
    );

    const isDrink = (item) => {
      const name =
        (item.category && item.category.name) || item.category_name || "";
        ...
        <div className="px-4 pt-4 pb-6 space-y-5 overflow-y-auto flex-1">
          ...
          <section className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              {drinkItems.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center justify-between rounded-xl border-b-2 border-gray-100 hover:border-orange-300 hover:bg-orange-50/50 transition-all h-20 pr-4 mb-5 ${drinkItems.indexOf(item) === drinkItems.length - 1 ? "mb-0" : ""}`}
                >
                  <div className="flex gap-3 h-full">
                    <div className="w-24 rounded-l-xl self-stretch overflow-hidden">
                      <img
                        src={
                          item.images?.[0] ||
          ...
          <section className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            ...
            <div>
              {sideItems.length === 0 && (
                <p className="text-xs text-gray-400">
                  No extra sides available right now.
                </p>
              )}
              {sideItems.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center justify-between rounded-xl border-b-2 border-gray-100 hover:border-orange-300 hover:bg-orange-50/50 transition-all h-20 pr-4 mb-5 ${sideItems.indexOf(item) === sideItems.length - 1 ? "mb-0" : ""}`}
                >
                  <div className="flex gap-3 h-full">
                    <div className="w-24 rounded-l-xl self-stretch overflow-hidden">
                      <img
          ...
          <div className="pt-4 border-t-2 border-gray-100">
            <div className="flex items-center justify-between mb-5 bg-orange-50 rounded-xl p-4">
